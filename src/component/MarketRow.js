...
class MarketRow extends React.Component{
  ...
  return(<tr>
      <td style={{fontWeight:'700'}}>
        <Link to={'/exchange/' + symbol}>{symbol}
       </Link>
     </td>
      <td>
        <Link to={'/exchange/' + symbol}>{avg}</Link>
      </td>
      <td>
        <Link to={'/exchange/' + symbol}>{vol}
      </Link>
    </td>
      <td>{check > 0 ?
      <Link to={'/exchange/' + symbol}>
      <span style={{fontWeight:'700'}} className="text-success">{daylichangePre}%</span></Link>:
      <Link to={'/exchange/' + symbol}>
      <span style={{fontWeight:'700'}} className="text-danger">{daylichangePre}%</span></Link>}</td>
       </tr>)
  }
}
